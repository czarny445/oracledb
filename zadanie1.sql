...
BEGIN
  ...
  IF (BLOKADA = 'N') THEN
    ...
    END IF;
  ELSE
    DBMS_OUTPUT.PUT_LINE('Zamówienie zostalo ju¿ zablokowane');
  END IF;
...
-- Triger 2: Automatyczne zmienianie iloœci produktu przy usuniêciu
CREATE OR REPLACE TRIGGER DELETE_FROM_LINIE_ZAMOWIENIA 
BEFORE DELETE ON LINIA_ZAMOWIENIA FOR EACH ROW
BEGIN
  UPDATE PRODUKT x SET ILOSC_SZTUK = x.ILOSC_SZTUK + :old.ILOSC WHERE x.PK = :old.ZAMOWIENIE;
END;

-- Triger 3: Automatyczne zmienianie iloœci produktu przy dodawaniu
CREATE OR REPLACE TRIGGER INSERT_FROM_LINIE_ZAMOWIENIA 
BEFORE INSERT ON LINIA_ZAMOWIENIA FOR EACH ROW
BEGIN
  UPDATE PRODUKT x SET ILOSC_SZTUK = x.ILOSC_SZTUK - :old.ILOSC WHERE x.PK = :old.ZAMOWIENIE;
END;

-- Wywoanie procedur 1,2,3

SET SERVEROUTPUT ON;
BEGIN
  DODAJ_PRODUKT_DO_ZAMOWIENIA('1','1','2');
  USUN_PRODUKT_Z_ZAMOWIENIA('1', '1');
  DODAJ_PRODUKT_DO_ZAMOWIENIA('1','1','2');
  ZABLOKUJ_ZAMOWIENIE('1');
  DODAJ_PRODUKT_DO_ZAMOWIENIA('1','1','2');
  USUN_PRODUKT_Z_ZAMOWIENIA('1', '1');
END;